...
namespace Shadow
{
    ...
    OpenGLShader::OpenGLShader(const std::string& filepath)
    {
        ...
        SD_CORE_ASSERT(shaderSources.size() != 0, "Shader not find !");
    ...
    OpenGLShader::OpenGLShader(const std::string& name, const std::string& vertexStr, const std::string& fragmentStr) : m_Name(name)
    {
        std::unordered_map<GLenum, std::string> sources;
        sources[GL_VERTEX_SHADER] = vertexStr;
        sources[GL_FRAGMENT_SHADER] = fragmentStr;
        Compile(sources);
    }
    ...
    {
        // 以二进制读方式打开文件
        // 跳跃到文件末尾的0偏移位置，读取到当前文件流的大小，并将string result置为文件大小
    ...
    std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
    {
        ...
        while (pos != std::string::npos)
        {
            ...
            // 从下一行开始找下一个Type，目的是判断当前 TypeShader 应该读到哪里
            // 判断成功后，写入shaderSources
